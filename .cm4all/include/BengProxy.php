...
class BengProxyHandler {
    ...
    function isInErrorState() {
        return $this->response === FALSE || (defined("CURLINFO_RESPONSE_CODE") && isset($this->curlHandle) && curl_getinfo($this->curlHandle, CURLINFO_RESPONSE_CODE) >= 500);
    }
    ...
    function createBackendUri($host) {
        ...
        if (preg_match('@^/\.cm4all/widgetres(?:\.php)?(/.*)?$@', $this->requestUri, $matches)) {
            return $host . "/.cm4all/widgetres" . $matches[1];
        }
        ...
        if (preg_match('@^/\.cm4all/handler(?:\.php)?(/.*)?$@', $this->requestUri, $matches)) {
            if (strpos($matches[1], "/xftcontroller.html") === 0) {
                return $host . "/res/js/lib/XFrameTunnel/controller.html" . substr($matches[1], 19);
            }
        }
